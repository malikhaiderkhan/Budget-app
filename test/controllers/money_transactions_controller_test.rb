...
class MoneyTransactionsControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should create money_transaction' do
    assert_difference('MoneyTransaction.count') do
      post money_transactions_url,
           params: { money_transaction: { amount: @money_transaction.amount, name: @money_transaction.name,
                                          user_id: @money_transaction.user_id } }
    end
  ...
  test 'should update money_transaction' do
    patch money_transaction_url(@money_transaction),
          params: { money_transaction: { amount: @money_transaction.amount, name: @money_transaction.name,
                                         user_id: @money_transaction.user_id } }
